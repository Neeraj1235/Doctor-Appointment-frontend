...
function AppointmentDetail() {
  ...
  const fetchData = useCallback(async () => {
    try {
      let res = await fetch(
        "https://doctor-appointment-backend-7hsb.onrender.com/doctor/appointment/" +
          id,
        {
          headers: {
  ...
  const submitHandler = async () => {
    let res = await fetch(
      "https://doctor-appointment-backend-7hsb.onrender.com/doctor/edit-appointment/" +
        id,
      {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({
          remarks:
            remarksRef.current.value === "" ? "Nil" : remarksRef.current.value,
        }),
      }
    );
    if (res.status === 200) {
  ...
  return (
    <>
      ...
      <Container fluid="sm" className="my-5 ">
        <h3 className="h3 mb-5 fw-bold fs-5">
          Appointment <span className="text-primary ps-1">#{id}</span>
        </h3>

        {isDataLoaded && (
          <Card className="rounded-5 p-3 border-2 h-100 ">
            <Card.Header className="whiteBg">
              <h3 className="d-inline-block fw-bold fs-3">Patient Info</h3>
              {isCompleted && isDataLoaded && (
                <Card.Text className="d-block text-success  float-sm-end d-sm-inline-block">
            ...
            <Card.Body style={{ fontSize: "18px" }}>
              <Row className="mb-2 fw-bold fs-5">
                <Col>Name</Col>
                <Col className="text-secondary fw-bold fs-5">{patientName}</Col>
              </Row>
              <Row className="mb-2 fw-bold fs-5">
                <Col>Age</Col>
        ...
        {!isCompleted && isDataLoaded && (
          <>
            <Card className="rounded-5 p-3 border-2 h-100 mb-5 mt-4">
              ...
              <Card.Body>
                <Form>
                  <Form.Label className="h3 mb-4 fw-bold fs-5">
                    Remarks
                  </Form.Label>
                  <Form.Control
                    as="textarea"
