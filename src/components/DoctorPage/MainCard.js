...
function MainCard(props) {
  ...
  const fetchAppointmentsTime = async () => {
    try {
      let res = await fetch(
        "https://doctor-appointment-backend-7hsb.onrender.com/doctor/get-appointmentsTime",
        {
          method: "POST",
          headers: {
    ...
    try {
      if (props.docData) {
        if (firstRender) {
          ...
          if (today > minDateObj) setDate(moment(today).format("YYYY-MM-DD"));
          else setDate(docData.availability.startDate);
    ...
    <>
      <Card className="rounded-5 p-3 border-2">
        <Card.Header className="whiteBg">
          <Row className="justify-content-center">
            ...
            <Col xs="12" md="5" lg="4" className="mb-3">
              {props.isAvailabilityAdded && props.isLoaded && (
                <Form.Control
                  type="date"
                  max={maxDate}
                  min={minDate}
        ...
        <Card.Body className="p-0 p-xl-3">
              ...
              <Container fluid className="p-0 w-100 mx-auto">
                {timeSlotArray.map((slot, idx) => (
                  <ToggleButton
                    ...
                    style={{ margin: "2px" }}
                    onClick={() => {
                      openAppointmentHandler(slot.id);
                    }}
                  >
                    {slot.time}
                  </ToggleButton>
                ...
                <Row>
                  <Col className="text-center">
                    <button
                      className="btn btn-outline-dark rounded-4 m-2  px-4"
                      type="button"
          ...
          {!props.isLoaded && props.isAvailabilityAdded && (
            <MediumPlaceHolder />
          )}
          {!props.isAvailabilityAdded && (
            <Card.Text className="text-center text-muted px-5">
              Add your availability first
            </Card.Text>
          )}
        </Card.Body>
      </Card>
