...
function SubCard(props) {
  ...
  useEffect(() => {
    try {
      setStartdate(
        moment(props.docData.availability.startDate).format("MMM DD, YYYY")
      );
      setEndDate(
        moment(props.docData.availability.endDate).format("MMM DD, YYYY")
      );
      setStartTime(
        moment(props.docData.availability.startTime).format("h:mm A")
      );
  ...
  return (
    <>
      ...
      <Card className="rounded-5 p-3 border-2 h-100 ">
        <Card.Header as="h3" className="whiteBg">
          {props.title}
        </Card.Header>
        <Card.Body>
          ...
          props.title === "Your Availability" ? (
            <>
              ...
              <Row className="mb-2">
                ...
                <Col className="text-secondary px-0">
                  {startTime} - {endTime}
                </Col>
              </Row>
            </>
