...
function AppointmentList() {
  ...
  const fetchData = async () => {
    try {
      let res = await fetch(
        "https://doctor-appointment-backend-7hsb.onrender.com/patient/appointments",
        {
          headers: {
  ...
  const cancelBookingHandler = async (id) => {
    try {
      ...
      let res = await fetch(
        "https://doctor-appointment-backend-7hsb.onrender.com/patient/cancel-appointment/" +
          id,
        {
          method: "DELETE",
          headers: {
  ...
  if (bookingData.length) {
            ...
            bookingData.map((appointment, i) => {
              return (
                <Row key={i}>
                  ...
                  <Col xs={7} className="text-start fs-5">
                  ...
                  <Col className="ms-5 mt-3 mt-sm-0  mb-4">
                    <OverlayTrigger
                      ...
                      placement="right"
                      overlay={
                        <Tooltip id={`tooltip-'bottom'`}> Reschedule</Tooltip>
                      }
                    >
                      <Button
                        className="btn-secondary m-1  btnInList"
                        disabled={cancellationLoading}
                        onClick={() => {
                          rescheduleHandler(appointment._id);
                        }}
                      >
                    ...
                    <OverlayTrigger
                      ...
                      placement="right"
                      overlay={
                        <Tooltip id={`tooltip-'bottom'`}> Cancel</Tooltip>
                      }
                    >
                      <Button
                        className="btn-danger m-1 btnInList"
                        disabled={cancellationLoading}
                        onClick={() => {
                          cancelBtnHandler(appointment._id);
                        }}
                      >
    ...
    return (
      <Card.Text className="text-muted text-center p-5 fw-bolder fs-5">
