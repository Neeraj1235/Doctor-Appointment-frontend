...
function SignupDoctor() {
  ...
  const SubmitHandler = async (event) => {
    ...
    if (purchaseCode !== "9634-7895-1236-7520") {
    ...
    try {
      setIsLoading(true);
      let res = await fetch(
        "https://doctor-appointment-backend-7hsb.onrender.com/doctor/signup",
        {
          method: "POST",
  ...
  return (
    <Form onSubmit={SubmitHandler}>
      <h4 className="text-center pb-3 text-secondary fw-dark fs-2">
        Sign up Form
      </h4>
      <FloatingLabel
      ...
      <div className="text-center">
        <Form.Label className="ps-2 text-secondary fw-bolder fs-4">
          Purchase Key
        </Form.Label>
        <br />
        <Form.Control
          type="text"
          value={pc1}
          onChange={(event) => pc1Handler(event)}
          required
          maxLength={4}
          id="code1"
          placeholder="xxxx"
          className="signup__purchaseCodeInp"
        />
        <Form.Control
          type="text"
          maxLength={4}
          ...
          className="signup__purchaseCodeInp"
          value={pc2}
          onChange={(event) => pc2Handler(event)}
          required
        />
        <Form.Control
          type="text"
          ...
          className="signup__purchaseCodeInp"
          value={pc3}
          onChange={(event) => pc3Handler(event)}
          required
        />
        <Form.Control
          type="text"
          ...
          className="signup__purchaseCodeInp"
          value={pc4}
          onChange={(event) => pc4Handler(event)}
          required
        />
        <br />
        <br />
        <Form.Text className="pt-2 text-secondary fw-bolder">
          Not Purchased the key?{" "}
          <Link
            className=" signup__signupBtn ps-1 link-primary fw-bolder"
            to={"/doctor/purchase"}
          >
            Purchase Now
          </Link>
      ...
      <div className="d-grid gap-2 col-6 mx-auto mt-4">
        <Button
          variant="primary fw-bolder fs-5"
          type="submit"
          disabled={isLoading}
        >
          {isLoading ? (
