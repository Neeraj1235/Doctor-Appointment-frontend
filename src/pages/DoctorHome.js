...
function DoctorHome() {
  ...
  const fetchData = async () => {
    try {
      let res = await fetch(
        "https://doctor-appointment-backend-7hsb.onrender.com/doctor/get-doctor",
        {
          headers: {
    ...
    <>
      ...
      <Container className="mt-5 pt-3">
        ...
        <MainCard
          docData={docData}
          isLoaded={isLoaded}
          isAvailabilityAdded={isAvailabilityAdded}
          setIsLoaded={setIsLoaded}
        />
      </Container>
      <Container className="mb-5 mt-3 ">
        <Row>
          <Col md className="mt-3 fw-bold fs-6">
            <SubCard
              title="Your Availability"
              isLoaded={isLoaded}
              docData={docData}
              isAvailabilityAdded={isAvailabilityAdded}
              onAvailHandler={setAvailabilityAdded}
            />
          </Col>
          <Col md="7" className="mt-3 fw-bold fs-5">
            <SubCard
              title="Notifications"
              isAvailabilityAdded={isAvailabilityAdded}
              isLoaded={isLoaded}
