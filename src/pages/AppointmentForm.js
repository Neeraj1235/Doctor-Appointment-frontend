...
function AppointmentForm() {
  ...
  const fetchDataForReschedule = useCallback(async () => {
    try {
      let res = await fetch(
        "https://doctor-appointment-backend-7hsb.onrender.com/patient/appointment/" +
          id,
        {
          headers: {
  ...
  const fetchDoctorsData = useCallback(async () => {
    try {
      let res = await fetch(
        "https://doctor-appointment-backend-7hsb.onrender.com/doctor/get-doctors",
        {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
          },
  ...
  const SubmitHandler = async (event) => {
    ...
    try {
      setIsLoading(true);
      let url =
        "https://doctor-appointment-backend-7hsb.onrender.com/patient/book-appointment";
      let method = "POST";
      if (id) {
        url =
          "https://doctor-appointment-backend-7hsb.onrender.com/patient/edit-appointment/" +
          id;
        method = "PUT";
      }
  ...
  const fetchAppointmentsTime = async (id, selectedDate) => {
    try {
      let res = await fetch(
        "https://doctor-appointment-backend-7hsb.onrender.com/patient/get-appointmentsTime",
        {
          method: "POST",
          headers: {
            "content-Type": "application/json",
  ...
  const setAllFields = (doctor) => {
    let selectedDocData;
    if (id) selectedDocData = doctor;
    else selectedDocData = docData.filter((doc) => doc.name === doctor)[0];
  ...
  return (
    <>
      ...
      <Container fluid="sm" className="mt-4">
        {isLoaded && (
          <>
            <h3 className="h3 mb-5 fw-bolder fs-3">Appointment Form</h3>
            <Form onSubmit={SubmitHandler}>
              <Form.Label className="text-center fw-bold text-muted mb-2 fs-5">
                Doctor
              </Form.Label>
              <Form.Select
              ...
              <Form.Group className="mb-5" controlId="appointmentDate">
                ...
                <div className="enableOnClick__container">
                  <div
                    onClick={clickDisabledHandler}
                    className={!isDocSelected ? "enableOnClick" : ""}
                  />
                  <Form.Control
                    type="date"
                ...
                {doctor !== " " && (
                  <Form.Text className="text-muted fw-bold">
                    Note. that Dr.{doctor} will be available only from{" "}
                    <span className="text-primary px-1">
                      {changeDateFormat(minDate)}
                    </span>{" "}
                    till{" "}
                    <span className="text-primary px-1">
                      {changeDateFormat(maxDate)}
                    </span>
                  </Form.Text>
                )}
              </Form.Group>

              {isDocSelected && isDateSelected && (
                <div className="mb-5">
                  ...
                  {timeSlotArray.length !== 0 ? (
                    ...
                    <div className="text-center mt-3mb-5">
              ...
              <Form.Group className="mb-5" controlId="Reason">
                ...
                <div className="enableOnClick__container">
                  ...
                  />
                  <Form.Control
                    disabled={id || !isDocSelected ? true : false}
                    type="text"
                    placeholder="Reason"
                    value={reason}
              ...
              {id && (
                <div className="d-grid gap-2 col-6 mx-auto mt-3 mb-5">
                  <Button
                    variant="secondary fw-bold fs-5"
                    type="button"
